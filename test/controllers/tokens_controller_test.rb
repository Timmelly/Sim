...
class TokensControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create token" do
    assert_difference('Token.count') do
      post tokens_url, params: { token: { api_action: @token.api_action, date_end: @token.date_end, token: @token.token } }
    end
  ...
  test "should update token" do
    patch token_url(@token), params: { token: { api_action: @token.api_action, date_end: @token.date_end, token: @token.token } }
